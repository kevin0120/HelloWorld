...
func (h *HTTPDHandler) NewAccessLogger() *log.Logger {
	s := &StaticLevelHandler{
		l:     h.l.With(String("service", "httpd_access_log")),
		level: llDebug,
	}
...
func (h *ProduceHandler) Error(msg string, err error) {
	h.l.Error(msg, Error(err))
}

func (h *ProduceHandler) Debug(msg string) {
	h.l.Debug(msg)
}

func (h *ProduceHandler) Info(msg string) {
	h.l.Info(msg)
}

// HmiHandler hmi handler
...
func (h *RedisHandler) Debug(msg string) {
	h.l.Debug(msg)
}

func (h *RedisHandler) Info(msg string) {
	h.l.Info(msg)
}

func (h *RedisHandler) Close() {
	h.l.Info("redis server closing")
}

func (h *RedisHandler) Closed() {
	h.l.Info("redis server closed")
}

func (h *RedisHandler) Disconnect(id string) {
	h.l.Info("redis Connection disconnected", String("ID", id))
}
...
// ECIOHandler ECIO
type ECIOHandler struct {
	l Logger
}

func (h *ECIOHandler) Error(msg string, err error) {
	h.l.Error(msg, Error(err))
}

func (h *ECIOHandler) Debug(msg string) {
	h.l.Debug(msg)
}

func (h *ECIOHandler) LocalIOStart() {
	h.l.Info("Local IO Start!!!")
}

func (h *ECIOHandler) LocalIOStop() {
	h.l.Info("Local IO Stop!!!")
}

func (h *ECIOHandler) Info(msg string) {
	h.l.Info(msg)
}

// OpenProtocolHandler Openprotocol
...
func (h *TighteningDeviceHandler) Info(msg string) {
	h.l.Info(msg)
}
...
func (h *CustomizeHandler) Errorf(format string, v ...interface{}) {
	msg := fmt.Sprintf(format, v...)
	err := errors.New(msg)
	h.Error(msg, err)
}
func (h *CustomizeHandler) Warnf(format string, v ...interface{}) {
	msg := fmt.Sprintf(format, v...)
	h.Info(msg)
}
func (h *CustomizeHandler) Debugf(format string, v ...interface{}) {
	msg := fmt.Sprintf(format, v...)
	h.Debug(msg)
}

type CVIMonitorHandler struct {
	l Logger
}

func (h *CVIMonitorHandler) Error(msg string, err error) {
	h.l.Error(msg, Error(err))
}

func (h *CVIMonitorHandler) Debug(msg string) {
	h.l.Debug(msg)
}

func (h *CVIMonitorHandler) Info(msg string) {
	h.l.Info(msg)
}

type CVINetWebHandler struct {
	l Logger
}

func (h *CVINetWebHandler) Error(msg string, err error) {
	h.l.Error(msg, Error(err))
}

func (h *CVINetWebHandler) Debug(msg string) {
	h.l.Debug(msg)
}

func (h *CVINetWebHandler) Info(msg string) {
	h.l.Info(msg)
}
