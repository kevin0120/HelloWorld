...
func (s *Rush) Pay(in int64) error {
	...
	//session.Rollback()
	//sql1 := fmt.Sprintf("update  `money` set rmb = rmb-%d where rmb = %d" , in, m.RMB)
	sql1 := fmt.Sprintf("insert into `payrecord` (rmb) select (%d) where (select rmb from money where id =1)>=%d;"+
		"update  `money` set rmb = rmb-%d where rmb-%d>=0", in, in, in, in)
	eff, err1 := session.Exec(sql1)
	if err1 != nil {
		fmt.Println(err1)
	}

	if a, _ := eff.RowsAffected(); a == 0 {
